...
APPLICATION_NAME = "Electronic Catalog Application"
...
@app.route('/category/<int:category>/item/JSON')
def onecategoryJSON(category):
...
def editCategory(category_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('New Category %s Successfully Edited' % editedCategory.name)
            return redirect(url_for('showCategory'))
    else:
...
def editItem(category_id, item_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit catalog items. Please create your own category in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
